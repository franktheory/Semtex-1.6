...
#define EnumToString(x) #x
	...
	//---------------------------------------------------------------------------
	HotkeyRightClickEvent &HotkeyControl::GetHotkeyRightClickEvent() {
		return hotkeyRightClickEvent_;
	}
	...
	bool HotkeyControl::ProcessMouseMessage( const MouseMessage &mouse ) {
		if( mouse.GetState() == MouseState::Down ) {
			if( mouse.GetButton() == MouseButton::Right && isEnabled_ && !isFocused_ ) {
				if( Intersection::TestRectangle( absoluteLocation_, GetSize(), mouse.GetLocation() ) ) {
					hotkeyRightClickEvent_.Invoke( this );
					return true;
				}
			}
