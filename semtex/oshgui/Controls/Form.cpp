...
namespace {
	constexpr uint8_t k_form_bg_alpha = 70;
	constexpr uint8_t k_accent_red = 210;
	constexpr uint8_t k_accent_green = 45;
	constexpr uint8_t k_accent_blue = 45;
	constexpr int k_accent_bar_height = 2;
	constexpr int k_accent_bar_inset = 2;
	constexpr int k_accent_bar_top = -2;
	constexpr int k_accent_bar_padding = 7;

	void layout_caption_title( OSHGui::Label *title, int bar_width, int caption_bar_y ) {
		if( title == nullptr ) {
			return;
		}

		const int title_y = k_accent_bar_top + k_accent_bar_height + k_accent_bar_padding - caption_bar_y;
		title->SetLocation( ( bar_width - title->GetWidth() ) / 2, title_y );
	}
}
...
namespace OSHGui {
	...
	Form::Form()
		...
		containerPanel_->SetLocation( Drawing::PointI( DefaultBorderPadding,
		                                               CaptionBar::DefaultCaptionBarHeight - 7 ) );
	...
	void Form::SetSize( const Drawing::SizeI &size ) {
		...
		const auto container_top = CaptionBar::DefaultCaptionBarHeight - 7;
		containerPanel_->SetSize( Drawing::SizeI(
			size.Width - DefaultBorderPadding * 2,
			size.Height - container_top - DefaultBorderPadding ) );
	}
	...
	void Form::PopulateGeometry() {
		...
		g.FillRectangle( Color::FromARGB( k_form_bg_alpha, 0, 0, 0 ), RectangleF( PointF( -6, -10 ), GetSize() + SizeF( 12, 16 ) ) );
		g.FillRectangle( Color::FromARGB( k_form_bg_alpha, 18, 18, 20 ), RectangleF( PointF( -5, -9 ), GetSize() + SizeF( 10, 14 ) ) );
		g.FillRectangle( Color::FromARGB( k_form_bg_alpha, 8, 8, 10 ), RectangleF( PointF( -4, -8 ), GetSize() + SizeF( 8, 12 ) ) );
		g.FillRectangle( Color::FromARGB( k_form_bg_alpha, 18, 18, 20 ), RectangleF( PointF( 0, -4 ), GetSize() + SizeF( 0, 4 ) ) );
		g.FillRectangle( Color::FromARGB( k_form_bg_alpha, 0, 0, 0 ), RectangleF( PointF( 1, 1 ), GetSize() - SizeF( 2, 2 ) ) );

		const auto accent = Color::FromARGB( 255, k_accent_red, k_accent_green, k_accent_blue );
		const auto bar_width = static_cast< float >( GetSize().Width - k_accent_bar_inset * 2 );

		g.FillRectangle( accent, PointF( k_accent_bar_inset, k_accent_bar_top ), SizeF( bar_width, k_accent_bar_height ) );

		const auto title_loc = captionBar_->GetTitleLocationInForm();
		const auto title_size = captionBar_->GetTitleSize();
		const auto bottom_bar_top = static_cast< float >( title_loc.Y + title_size.Height + k_accent_bar_padding );

		g.FillRectangle( accent, PointF( k_accent_bar_inset, bottom_bar_top ), SizeF( bar_width, k_accent_bar_height ) );
	...
	Form::CaptionBar::CaptionBar() {
		...
		titleLabel_->SetFont( Drawing::FontManager::LoadFont( "Verdana", 16.f, true ) );
	...
	void Form::CaptionBar::SetSize( const Drawing::SizeI &size ) {
		...
		layout_caption_title( titleLabel_, size.Width, GetLocation().Y );
	...
	void Form::CaptionBar::SetText( const Misc::AnsiString &text ) {
		...
		layout_caption_title( titleLabel_, GetWidth(), GetLocation().Y );
		GetParent()->Invalidate();
	}

	//---------------------------------------------------------------------------
	Drawing::PointI Form::CaptionBar::GetTitleLocationInForm() const {
		return GetLocation() + titleLabel_->GetLocation();
	}

	//---------------------------------------------------------------------------
	Drawing::SizeI Form::CaptionBar::GetTitleSize() const {
		return titleLabel_->GetSize();
	}
